...
void matmul_scalar_mt_worker(void* arg) {
...
void matmul_hvx_mt_worker(void* arg) {
    ...
    // Release HVX context so other threads can use it
    qurt_hvx_unlock();
    qurt_thread_exit(0);
}


uint64_t matmul_hvx_mt_parallel(float* A, float* B, float* C, int n, int k, int num_threads) {
    ...
    printf("HVX + dynamic multithreaded matmul: n=%d, k=%d, threads=%d\n", n, k, num_threads);
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        int ret = qurt_thread_create(&threads[t], &attrs[t], matmul_hvx_mt_worker, &contexts[t]);
        if (ret != QURT_EOK) {
...
int main(int argc, char** argv) {
    ...
    printf("Starting HVX + multithreaded parallel matmul...\n\n");
    uint64_t cycles = matmul_hvx_mt_parallel(A, B, C, n, k, num_threads);
